...
def predict_sound(AUDIO, SAMPLE_RATE, plot=False):
    results = []
    ...
    splitted_audio_data = tf.signal.frame(clip, sample_rate, sample_rate, pad_end=False, pad_value=0)
...
while (1):
    ...
    if meu_nome in result:
        ...
        if result in comandos[5]:
            mode_control = True
            play_sound('n2.mp3')
            speak('Modo do análise de emoçaõ foi ativado')
